<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
            <meta charset="utf-8">
                <meta content="width=device-width, initial-scale=1, shrink-to-fit=no" name="viewport">
                    <link href="assets\bootstrap\css\bootstrap.min.css" rel="stylesheet">
                        <link href="assets\css\fontawesome-free\css\all.min.css" rel="stylesheet">
                            <link href="assets\css\style.css" rel="stylesheet">
                                <title>
                                    Data Editor
                                </title>
                            </link>
                        </link>
                    </link>
                </meta>
            </meta>
        </meta>
    </head>
    <body>
        <!-- Navbar -->
        <nav class="navbar navbar-expand-lg navbar-light" style="background-color: #c9c7c7;">
            <a class="navbar-brand font-weight-bold" href="index.php">
                <i class="fas fa-edit">
                </i>
                DATA EDITOR
            </a>
            <button aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation" class="navbar-toggler" data-target="#navbarSupportedContent" data-toggle="collapse" type="button">
                <span class="navbar-toggler-icon">
                </span>
            </button>
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item active">
                        <a class="nav-link" href="cari.php">
                            PENCARIAN EDITOR
                        </a>
                    </li>
                    <li class="nav-item active">
                        <a class="nav-link" href="tampil.php">
                            TAMPILKAN EDITOR
                        </a>
                    </li>
                    <li class="nav-item active">
                        <a class="nav-link" href="about.php">
                            ABOUT ME
                        </a>
                    </li>
                </ul>
            </div>
        </nav>
        <!-- End Navbar -->
        <!-- Jumbotron -->
        <div class="container text-center mt-5">
            <p class="display-4 m-0">
                Risma Kurniawaty
            </p>
            </p>
            <img src="assets\\img\\Ri.jpeg" width="20%">
            </center>
            <hr>
                <p class="text-muted mr-1" style="font-size: 25px">
                    <i class="fas fa-envelope-open-text">
                    </i>
                    risma.k@mhs.unsyiah.ac.id
                </p>
                <p class="text-muted mr-1" style="font-size: 25px">
                    <i class="fas fa-map-marked-alt">
                    </i>
                    Darussalam, Banda Aceh
                    <br>
                        Informatika
                    <br>
                        Universitas Syiah Kuala
                    </br>
                </p>
            </hr>
        </div>
        <script src="assets\bootstrap\js\bootstrap.min.js">
        </script>
        <script src="assets\bootstrap\js\jquery.min.js">
        </script>
    </body>
</html>
